...
use crate::{error::{print_error_and_backtrace, CrawlerError}, parser::find_anchors, crawler::Crawler};
...
impl UrlWorker {
        ...
        for link_url in find_anchors(document.as_str(), worker_index, PARSER_WORKER_COUNT) {
            match Self::parser_worker_handle_link(&previous_url, link_url.as_str()) {
                Ok(link_url) => {
                    ...
                    tokio::spawn(async move {
                        if let Err(err) = crawler.start_job(Some(&previous_url_clone), &link_url).await {
    ...
    fn parser_worker_handle_link(
        ...
        if !have_same_base(previous_url, &link_url) {
            let err = CrawlerError::LinkUrlDoesNotMatchBaseUrl(previous_url.to_string(), link_url.to_string()).into();
            return Err(err);
        }
        Ok(link_url)
    }
