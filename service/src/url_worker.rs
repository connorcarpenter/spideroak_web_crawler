...
use crate::{error::{print_error, CrawlerError}, parser::find_anchors, crawler::Crawler};

const URL_MAX_STALE_MINUTES: i64 = 5;
...
impl UrlWorker {
    ...
    fn parser_worker(
        ...
        for link_url in find_anchors(document.as_str(), worker_index, PARSER_WORKER_COUNT) {
            match Self::parser_worker_handle_link(&base_url, link_url.as_str()) {
                Ok(link_url) => {
                    let crawler = crawler.clone();
                    tokio::spawn(async move {
                        if let Err(err) = crawler.start_job(&link_url).await {
                            print_error(err);
                        }
                    });
                }
    ...
    fn parser_worker_handle_link(
        ...
        let link_url = match Url::parse(link_url) {
            Ok(url) => url,
            Err(ParseError::RelativeUrlWithoutBase | ParseError::RelativeUrlWithCannotBeABaseBase | ParseError::EmptyHost | ParseError::SetHostOnCannotBeABaseUrl) => {
                if let Ok(resolved_url) = base_url.join(link_url) {
                    resolved_url
                } else {
                    return Err(CrawlerError::FailedToResolveRelativeUrl(link_url.to_string()));
                }
            }
            Err(err) => {
        ...
        if !have_same_base(base_url, &link_url) {
            let err = CrawlerError::LinkUrlDoesNotMatchBaseUrl(base_url.to_string(), link_url.to_string()).into();
            return Err(err);
        }
        Ok(link_url)
    }
